...
function ThreadItem({ thread, isActive, onSelect, onDelete, onRename }) {
  ...
  const handleRenameSubmit = (e) => {
    e?.preventDefault();
    if (editValue.trim()) onRename(thread.id, editValue.trim());
    setIsEditing(false);
  };
              ...
              <div className="bg-red-950/40 border border-red-800/40 rounded-lg p-3">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 text-xs py-1.5 bg-red-600 hover:bg-red-500 text-white rounded-lg transition-colors"
                  >
                    Delete all
                  </button>
                  <button
